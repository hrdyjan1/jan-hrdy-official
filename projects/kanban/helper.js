...
export const addEntity = (state, entity, id) => {
  return pipe(set(['entities', id], entity), set('results', state.results.concat(id)))(state);
};

export const removeEntity = (state, id) => {
  return pipe(omit(`entities.${id}`), set('results', removeFromArray(state.results, id)))(state);
};
